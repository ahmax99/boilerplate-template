...
module "backend" {
  ...
  lambda_layers = [
    "arn:aws:lambda:${var.aws_region}:${data.aws_caller_identity.current.account_id}:layer:boilerplate-bun-runtime:3", // enabled after running deploy-bun-layer.sh
...
module "frontend" {
  ...
  lambda_code_s3_bucket = module.s3_lambda_code.bucket_name
  lambda_code_s3_key    = "${local.frontend_artifact_prefix}/lambda-frontend.zip"
