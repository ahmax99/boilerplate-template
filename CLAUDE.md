...
- Git hooks are managed by **Lefthook** (`lefthook.yml`, installed via the `prepare` script). Pre-commit runs `oxfmt`/`oxlint --fix` on staged TS/JS/YAML/MD/JSON and `terraform fmt -recursive` when `infra/terraform/**` HCL is staged; `commit-msg` runs commitlint; `pre-push` runs `turbo run check-types build --affected`; `post-commit` rebuilds the graphify graph.
