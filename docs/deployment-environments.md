...
| `DNS_ACCOUNT_ROLE_ARN`         | variable   | _(empty)_                                                                             | Role ARN in the domain-hosting account                                                 | `TF_VAR_dns_account_role_arn` — assumed for cross-account Route 53 writes into the root zone. Dev owns its own delegated subzone in-account, so it stays empty.                |
...
Both ARNs are constructed deterministically from account-ID variables rather than read from the other account's live state, so there's no bootstrap cycle — see "Three-Account Bring-Up Ordering" below.
...
| `vars.DNS_ACCOUNT_ROLE_ARN`           | variable | Manually set (non-secret) — role ARN in the domain-hosting account (see "Three-Account Bring-Up Ordering" below).                                                            |

The `dev` environment's equivalent checklist sets `vars.PROMOTION_GRANTEE_ACCOUNT_ID` (the **prod** account ID) instead of `SOURCE_ECR_ACCOUNT_ID`, and leaves `vars.DNS_ACCOUNT_ROLE_ARN` empty (dev's delegated zone lives in-account) — see "Three-Account Bring-Up Ordering" below for which account sets which.

## Three-Account Bring-Up Ordering

The root Route 53 zone (`root_domain`) lives in its own dedicated **domain account** — separate from dev and prod — so a broad IAM policy or a mis-scoped delete in either app account has no path to the DNS zone that both depend on. Dev and prod each still need their own account-level bootstrap (state bucket, OIDC provider, apply role — see `infra/terraform/bootstrap/README.md`) before the root module can apply into them. The cross-account ECR grants (dev → prod) and the DNS role assumption (prod → domain account) are built from account-ID/ARN **variables**, not from reading the other account's live Terraform state, so there is no real bootstrap cycle — the order below is a straight sequence, not a multi-pass dance:

1. **Provision the AWS Organization and its three member accounts** (dev, prod, domain) — out of scope for this repo's Terraform; assumed done via the Org's own bring-up process before step 2.
2. **Bootstrap the domain account** (domain-account admin credentials, outside this repo's root module — it holds no app resources): create the root hosted zone for `root_domain`, and an IAM role (e.g. `boilerplate-template-dns-cross-account-role`) whose trust policy allows the **prod** deploy role (`arn:aws:iam::<prod-account-id>:role/boilerplate-template-prod-github-actions-role` — deterministic, same pattern as the ECR grants) to assume it, with permissions scoped to `route53:GetHostedZone`, `route53:ListResourceRecordSets`, and `route53:ChangeResourceRecordSets` on that one zone's ARN, **plus** `route53:ListHostedZonesByName` (`Resource: "*"` — no ARN form exists) and `route53:GetChange` (`Resource: "arn:aws:route53:::change/*"`) — Terraform's `data.aws_route53_zone` lookup-by-name and its post-`ChangeResourceRecordSets` `INSYNC` polling use these two actions, and neither can be scoped to the zone's own ARN (this is the standard, AWS-documented exception for `GetChange`, since change IDs aren't zone-scoped — but they do have their own resource type, so scope to that rather than a bare `*`). Omitting them causes the first prod apply that exercises this provider to fail with `AccessDenied`, behind the reviewer gate. Record the role's ARN as `vars.DNS_ACCOUNT_ROLE_ARN` in the `prod` GitHub environment.
3. **Bootstrap the dev account** (`infra/terraform/bootstrap/`, dev-account admin credentials): creates the dev state bucket, then the dev-account OIDC provider + dev apply role (`bootstrap/README.md` steps S1–S3, dev workspace only).
4. **Bootstrap the prod account** (prod-account admin credentials, same bootstrap module, prod workspace): creates the prod state bucket, then the **prod-account's own** OIDC provider + prod apply role — the `github_oidc` module always creates the provider now, so this is a full bootstrap in the prod account too, not a "provider already exists" shortcut.
5. **Apply dev** (`vars/dev.tfvars` with `promotion_grantee_account_id` set to the prod account ID, `source_ecr_account_id` and `dns_account_role_arn` left empty): creates dev's ECR repos, each with a resource policy naming the (not-yet-existing) prod deploy role ARN as an allowed pull principal — safe because that ARN is deterministic, not read from prod's state. Also creates dev's own delegated Route 53 zone (`aws_route53_zone.delegated`), fully in-account — dev never touches the domain account.
6. **Delegate DNS for dev** (manual, cross-account — dev apply cannot write into the domain account's zone): read the `dev_zone_name_servers` output from step 5, then in the **domain** account create an `NS` record for `dev.<project>.<root_domain>` in the root zone pointing at those name servers. ACM validation for dev's certificate will not complete until this propagates (can take minutes).
7. **Apply prod** (`vars/prod.tfvars` with `source_ecr_account_id` set to the dev account ID, `promotion_grantee_account_id` left empty, `dns_account_role_arn` set to step 2's role ARN): creates the prod deploy role — its identity policy grants pull on dev's ECR ARNs (also deterministic), and dev's repo policy from step 5 already names this exact role ARN, so the ECR trust is live on both sides as soon as this apply completes. Prod's `data.aws_route53_zone.main` and its ACM validation record are read/written through the `aws.dns` provider, which assumes the step 2 role — no manual delegation needed on prod's side, only the one-time trust set up in step 2.
8. **Continue into "First bring-up" below** to wire up the `prod` GitHub environment and cut the first release.
