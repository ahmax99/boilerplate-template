...
- **App vs infra separation:** `deploy.yml` ships application images; `terraform-apply.yml` ships infrastructure. They share the trigger model but not a `needs` edge — don't couple them without a design discussion. Two deliberate, already-agreed exceptions exist for ordering only (never a direct `needs` across workflows, always via `wait-for-workflow-run` polling the other workflow's run): `wait-for-prod-infra` (tag pushes) and `wait-for-dev-infra` (branch pushes that also touch `infra/**`) block the respective deploy jobs until `terraform-apply.yml`'s apply for that environment concludes. A new coupling beyond these two still needs a design discussion.
