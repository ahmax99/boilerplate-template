...
> **IMPORTANT:** Always run commands using the project's package runner: `bunx shadcn@latest`, `pnpm dlx shadcn@latest`, or `bunx --bun shadcn@latest`. Check `packageManager` from project context to choose the right one. Examples below use `bunx shadcn@latest` but substitute the correct runner for the project.
...
```bash
bunx shadcn@latest apply [preset] [options]
```
...
bunx shadcn@latest add owner/repo/item --dry-run
...
Fuzzy search across registries. Also aliased as `bunx shadcn@latest list`.
...
`bunx shadcn@latest view owner/repo/item`.
...
> Use `bunx shadcn@latest apply --preset <code>` when overwriting an existing project's preset.

## Switching Presets

Ask the user first: **overwrite**, **merge**, or **skip** existing components?

- **Overwrite / Re-install** → `bunx shadcn@latest apply --preset <code>`. Overwrites all detected component files with the new preset styles. Use when the user hasn't customized components.
