...
**Auth flow** (`src/features/auth/`): OIDC with Cognito using `openid-client` + PKCE. State lives in **two** `iron-session` HttpOnly cookies — transient PKCE state in `auth_pkce`, and the completed session (holding only the refresh token) in `auth_session`. `src/proxy.ts` is the Next.js middleware that gates `PROTECTED_ROUTES` (redirects to login if `auth_session` is absent) — it only checks cookie _presence_, not validity; real verification happens in the backend. [`docs/diagrams/auth-system-flow.html`](../../docs/diagrams/auth-system-flow.html) draws the whole lifecycle on one canvas — both cookies, all five BFF entry points, Cognito, and JWT verify → CASL. Read it before changing anything in `features/auth/`, the BFF clients, or `proxy.ts`.

Four invariants hold that design together. Each exists because the alternative broke something:

- **Two cookies, not one — do not merge them back.** An earlier version kept PKCE state and the session in a single `auth_session` cookie. Because `proxy.ts` checks only presence, merely _starting_ a login wrote the cookie and looked authenticated — soft-locking the login page and letting half-authenticated visitors past the edge guard.
- **`auth_session` holds `refreshToken` and nothing else.** `sub` and email are always available from a freshly-minted ID token's claims or `getMe()`, so persisting them would only create a second, staleable source of truth.
- **The edge guard stays presence-only.** The middleware never decrypts the session — it is a routing convenience, not the security boundary. The backend's JWT verification is the real gate.
- **The ID-token hook must run before the SigV4 `signingHook`.** SigV4 claims the `Authorization` header for the AWS signature, so the Cognito ID token rides in `X-Id-Token` instead. See the comments in `lib/serverApiClient.ts`.
...
- `infra/` — `bootstrap/`, `backends/`, `vars/`, `modules/`. Deployment topology (CloudFront → Next BFF + S3 + Elysia Lambda + Cognito + Neon), the cache behaviors and the Lambda IAM guard are owned by `.claude/rules/infra.md`'s **Deployment topology** section — read it before changing any of them.
