...
- **`deploy-on-aws`** (MCP: `awsknowledge` / `awsiac` / `awspricing`) — AWS documentation search, IaC validation helpers, and pricing lookups. Use it for AWS service questions any time, and for cost estimates at `/spec` / `/plan` time on cost-shaped work — not only during `/implement`. Its `aws-architecture-diagram` skill is in scope for regenerating the diagrams under `docs/diagrams/`; the `deploy` / `elastic-beanstalk` skills are not (this repo's deployment is already defined in `infra/`).
